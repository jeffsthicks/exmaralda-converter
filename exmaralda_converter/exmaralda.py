...
class Timepoint:
    ...
    def __init__(self, time_stamp=-1, type='', ):
        ...
        :param time_index: index where this time stamp sits in the timeline
...
class Tier:
    ...
    get_event(t)
        Gets an event from the timestamp
    """
    ...
    def add_event(self, e):
        ...
        self.event_dict[e.start.time_id]=e
...
class timeline:
    ...
    def append(self,point:Timepoint):
        if point.get_time_id() in self.indexes.keys():
            print("Error: There are two timepoints with the same ID!")
        else:
            self.list.append(point)
            self.indexes[point.get_time_id()]=len(self.list)-1

    def get_index(self,time):
        if isinstance(time,str):
            return self.indexes[time]
        elif isinstance(time,Timepoint):
            return self.indexes[time.time_id]
    
    def get_Timepoint(self,data):
        if isinstance(data,str):
            return self.list[self.indexes[data]]
        else:
...
class ExmaraldaTranscript:
    ...
    def add_event(self, event, tier_id):
        ...
        if tier_id in self.tiers.keys():
            # make sure time points from event begin and end are in timeline by matching their time_id, then replace the timepoints in the event to match those of the timeline.
            if event.start.time_id not in self.timeline.time_ids():
                self.timeline.append(event.start)
            else: #reset the start and End to point to the timepoint in the timeline
                event.start=self.timeline.get_Timepoint(event.start.time_id)
            if event.end.time_id not in self.timeline.time_ids():
                self.timeline.append(event.end)
            else:
                event.end=self.timeline.get_Timepoint(event.end.time_id)
            #Once you've corrected the timepoints on the events to match the ones on the timeline, add the event to the tier.
    ...
    def print_timeline(self, indentation_level=0):
        ...
        if len(self.timeline.list) == 0:
            return rval+'</common-timeline>'
        rval += '\n'+'\n'.join(timepoint.pretty_print(indentation_level+1) for timepoint in self.timeline.list)
    ...
    def load(in_file):
        ...
        # load timelines
        for k,time_info in enumerate(root.iter('tli')):
            try: new_tp = Timepoint(time_stamp=time_info.attrib['time'])
            except: new_tp = Timepoint(time_stamp="0") 
            # TODO re-think autogenerated timestamps
            new_tp.time_id = time_info.attrib['id'][1:] # overwrite the autogenerated timestamp
            rval_transcript.timeline.append(new_tp)
